...
export const parser = StructuredOutputParser.fromZodSchema(
	z.object({
		isMovementReadQuery: z.boolean().describe("Query requires reading data from Movement blockchain"),
...
export const prompt = PromptTemplate.fromTemplate(
    ...
    2. Determine if the query requires Movement read operations, tweet/post write operations, or posting on X/Twitter
    ...
    - Movement Read Operations include: 
...
export const managerNode = async (state: typeof StateAnnotation.State) => {
	...
	const { isMovementReadQuery, isWriterQuery, isXPostQuery } = result
...
export const managerRouter = (state: typeof StateAnnotation.State) => {
	const { isMovementReadQuery, isWriterQuery, isXPostQuery } = state

	if (isMovementReadQuery) {
		return "movementRead"
